...
export async function renderWatermarkOutro(
  ...
  const brandSize = Math.round(height * 0.055)
  const tagSize = Math.round(height * 0.03)
  ...
  const vf = [
    `drawtext=text='Made with AtVe Animation':${font}:fontsize=${brandSize}:fontcolor=0xF5EBDC:x=(w-text_w)/2:y=${brandY}`,
