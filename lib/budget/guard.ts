...
export async function gateAndRecord(
  ...
  if (summary.level === "hard") {
    console.error(`[budget] HARD STOP — today $${summary.todayUsd.toFixed(2)}/${DAILY_BUDGET_USD}, month $${summary.monthUsd.toFixed(2)}/${MONTHLY_BUDGET_USD}; rejected ${provider}:${model}`)
    throw new BudgetExceededError(`daily/monthly ceiling reached`)
  }
  if (summary.level === "soft") {
