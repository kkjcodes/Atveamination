...
//    Container Apps' 240s ingress cap on multi-scene renders. Stay on the
//    bare `fal-ai/kokoro` alias which is fast and still supported.
//
...
// Kept for tests/future migration — voice ID prefix maps 1:1 with the
// language endpoint if we ever need to move off the bare alias.
...
export async function synthesizeKokoro(
  ...
  const result = await fal.subscribe(KOKORO_ENDPOINT, {
  ...
  if (!audioUrl) {
    throw new Error(`Kokoro (${KOKORO_ENDPOINT}) returned no audio URL. Response keys: ${Object.keys(d ?? {}).join(", ")}`)
  }
  return { audioUrl }
}
